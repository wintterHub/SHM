...
<body>
	...
	<div class="container">
		<div class="row mybox">
			<div class="detailed-panel">
				<div class="col-md-12 col-sm-12 col-xs-12">
					<blockquote>
						<div class="panel-heading">我发布的商品</div>
					</blockquote>
							...
							<table class="table table-hover" id="goods-table">
								...
								<tbody>
									<s:iterator value="#DataRows" status="status">
										<tr onclick="goodsDetailed('${id}')" class="goodsitem">
											<td><s:property value='#status.count' /></td>
											<td><img src="${images[0]}"></td>
											<td>${title}</td>
											<td>￥${price}</td>
											<td>
												<button onclick="edit('${id}')" id="btn-del"
				...
				<div class="modal-body">
					<div class="text-area">
						<div class="form-group form-inline">
							<div class="info-item">确定删除该商品吗</div>
						</div>
					</div>
				</div>
	...
	<script>
					...
					.click(
							function() {
								$
										.ajax({
											type : "post",
											url : "${pageContext.request.contextPath }/goods/goodsAction_del.action",
											data : {
												"id" : id
											},
		...
		function edit(id) {
			event.stopPropagation();//阻止事件冒泡
			window.location.href = "${pageContext.request.contextPath }/goods/goodsAction_editGoodsPage.action?id="
					+ id;
		}

		function goodsDetailed(id) {
			window.location.href = "${pageContext.request.contextPath }/goods/goodsAction_detailedPage.action?id="
					+ id;
		}
	</script>
	<%-- 	<s:debug></s:debug> --%>
